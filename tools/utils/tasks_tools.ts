...
export function runSequence(...sequence: any[]) {
  ...
  _scanDir(TASKS_PATH, taskName => tasks.push(taskName));
...
// Private Methods
// ---------------
function _registerTask(taskName: string, fileName?: string, option: string = ''): void {
  gulp.task(taskName, task(fileName || taskName, option));
}

function _scanDir(root: string, cb: (taskName: string) => void) {
